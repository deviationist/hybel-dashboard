...
export function PendingActions({ id, className, onActionClick }: PendingActionsProps) {
      ...
      <Card>
          ...
          <CardContent className="p-0">
            {isFetching ? (
              ...
              <p className="p-4 text-sm text-muted-foreground">No pending actions</p>
            ) : (
              <ItemGroup>
                  ...
                  return (
                    <div key={action.id}>
                      ...
                      <Item
                        ...
                        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onActionClick(action.unitId); } }}
                        role="button"
                        tabIndex={0}
                        aria-label={`${formatAddressShort(action.address)}: ${action.description}`}
                      >
                        <ItemMedia>
                          <Icon className={cn("size-5", severityConfig[action.severity])} />
                        </ItemMedia>
                        <ItemContent>
                          <ItemTitle>{formatAddressShort(action.address)}</ItemTitle>
