...
export function MetricCard({ className, children, label, value, onClick, icon: Icon }: MetricCardProps) {
  return (
    <Card
      className={cn(className, onClick && "cursor-pointer hover:ring-2 hover:ring-primary/20 transition-shadow")}
    ...
    >
      <CardContent className="flex gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xl font-bold text-foreground">{children ?? value}</p>
        </div>
        {onClick && (
